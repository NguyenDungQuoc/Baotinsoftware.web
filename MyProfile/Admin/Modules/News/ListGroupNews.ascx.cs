...
{
    public partial class ListGroupNews : UserControl
    {
        NewsBlogProvider nb = new NewsBlogProvider();
        ...
        private void BindData_GroupNews()
        {
            GridView_ListData.DataSource = rpt_ListCategories.DataSource = nb.LIST_CATEGORIES_NEWS();
        ...
        protected void GridView_ListData_OnRowCommand(object sender, GridViewCommandEventArgs e)
        {
            switch (e.CommandName)
            {
                case "IsActive":
                    ...
                    var k = Convert.ToBoolean(nb.GET_TABLE_GROUP_NEWS().Where(w => w.ID == id).FirstOrDefault().Active);
                    nb.UPDATE_IS_ACTIVE_TABLE_GROUP_NEWS(Convert.ToInt32(id), !k);
                    BindData_GroupNews();
                    break;
            }
        }               
        ...
        public void DeleteRecord()
        {
            ...
            nb.DELETE_RECORD_TABLE_GROUP_NEWS((string.IsNullOrEmpty(id) ? 0 : Convert.ToInt32(id)));
        }
        ...
        public void bindRecordToEdit()
        {
            try
            {
                ...
                if (idRecord != null)
                {
                    var g = nb.GET_TABLE_GROUP_NEWS().FirstOrDefault(w => w.ID == (string.IsNullOrEmpty(idRecord) ? 0 : Convert.ToInt32(idRecord)));
        ...
        public void insertRecordGroupProducts()
        {
            ...
            {
                nb.INSERT_TABLE_GROUP_NEWS(name, name.UnicodeFormat(),
            ...
            else
            {
                int pID;
                if (Convert.ToInt32(
                    nb.GET_TABLE_GROUP_NEWS()
                        .Where(w => w.ID == Convert.ToInt32(idGroupProduct))
                        .FirstOrDefault()
                        .ParentID) == 0)
                {
                    pID = Convert.ToInt32(idGroupProduct);
                }
                else
                {
                    pID = 0;
                ...
                nb.INSERT_TABLE_GROUP_NEWS(name, name.UnicodeFormat(),
                    string.IsNullOrEmpty(po) ? 0 : Convert.ToInt32(po), des.Trim(),
                    pID, !string.IsNullOrEmpty(active) && bool.Parse(active));
            }
        }
        ...
        public void updateRecordGroupProducts()
        {
            ...
            if (id != null)
            {
                ...
                nb.UPDATE_TABLE_GROUP_NEWS(Convert.ToInt32(id), name.Trim(), name.UnicodeFormat(),
        ...
        public void updateRecordPosition()
        {
            ...
            nb.UPDATE_POSITION_TABLE_GROUP_NEWS(Convert.ToInt32(id), string.IsNullOrEmpty(p) ? 0 : Convert.ToInt32(p));
        }
    }
}