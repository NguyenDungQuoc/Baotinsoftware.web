<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ProductOfCategories.ascx.cs" Inherits="MyProfile.Modules.Products.ProductOfCategories" %>
        ...
        <asp:Repeater ID="rpt_Pager" runat="server">
            ...
            <ItemTemplate>
                <li class="<%# Utility.GetClassDisabledPaging(Convert.ToBoolean(Eval("Enabled"))) + " " + Utility.GetClassActivePaging(PageIndex.ToString(),Eval("Value").ToString())  %>">
                    <a href="<%# "/san-pham/" + ProductProvider.Get_Url_Category(Convert.ToInt32(Request.QueryString["id"])) + Eval("Value","/page-{0}.html") %>"><%#Eval("Text") %></a>                                        
                </li>
            </ItemTemplate>
