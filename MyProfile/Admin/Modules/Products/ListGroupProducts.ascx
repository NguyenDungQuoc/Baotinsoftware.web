<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ListGroupProducts.ascx.cs" Inherits="MyProfile.Admin.Modules.Products.ListGroupProducts" %>
        ...
        <div class="box box-info">
            <div class="box-header no-padding-bottom">
                ...
                <div class="pull-right box-tools">
                    ...
                    <asp:LinkButton runat="server" ID="LkbSave" CssClass="btn btn-primary btn-flat marginLeft-10" 
                        OnClientClick="return confirm('Bạn có thực sự muốn lưu?')" OnClick="LkbSave_OnClick">
                        <i class="ace-icon fa fa-save bigger-120 orange"></i>
                        Saves
                    </asp:LinkButton>
                ...
                <asp:GridView ID="GridView_ListData" runat="server" AutoGenerateColumns="False" BackColor="White" BorderColor="#CCCCCC" BorderStyle="None" 
                    ...
                    <Columns>
                        ...
                        <asp:TemplateField HeaderText="Tên danh mục">
                            <ItemTemplate>
                                <a href="<%# Eval("ParentID") %>" style='<%# Convert.ToInt32(Eval("ParentID")) == 0 ? "color:#EC0700;" : "" %>' class="callmodal" pid="<%# Eval("ID") %>" title="<%# Eval("Name") %>">
                                    <%# Eval("Name") %>
                                </a>                                                                
                            </ItemTemplate>
                        ...
                        <asp:TemplateField HeaderText="Nổi bật">
                            <ItemTemplate>
                                <asp:LinkButton ID="lkbShow" CommandArgument='<%# Eval("ID") %>' CommandName="IsShow" runat="server" ToolTip='<%# Eval("IsHot") %>'>
                                    <%# Utility.GetLabelIsHot(Eval("IsHot"), "Hiển thị ra trang chủ", "Không hiển thị ra trang chủ") %>
                                </asp:LinkButton>                                            
                            </ItemTemplate>
                    ...
                    <div class="col-sm-5">                        
                        <select id="cmb_CategoriesParent" class="form-control" data-placeholder="-- Chọn --">
                            ...
                            <asp:Repeater runat="server" ID="rpt_ListCategories">
                                <ItemTemplate>
                                    <option prid="<%# Convert.ToInt32(Eval("ParentID")) == 0 %>" value="<%# Eval("ID") %>">
                                        <%# Eval("Name") %>
                                    </option>
                                </ItemTemplate>
                            </asp:Repeater>
...
<script type="text/javascript" src="/Theme/javascript/js_adminGroupProducts.js"></script>
